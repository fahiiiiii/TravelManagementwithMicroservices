...
register_model = api.model(
    ...
    {
        "name": fields.String(
            required=True, description="User full name", example="John Doe"
        ),
        "email": fields.String(
            required=True, description="User email address", example="john@example.com"
        ),
...
class LoginResource(Resource):
    ...
    def post(self):
        ...
        data = request.json
        user = next(
            (u for u in users_db.values() if u["email"] == data.get("email")), None
        )
    ...
    def post(self):
        ...
        try:
            ...
            decoded_token = jwt.decode(
                data["accessToken"], JWT_SECRET, algorithms=["HS256"]
            )
            user = next(
                (u for u in users_db.values() if u["id"] == decoded_token["id"]), None
            )
