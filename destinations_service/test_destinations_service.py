...
def mock_file_operations():
    ...
    with patch("app.save_destinations", side_effect=mock_save), patch(
        "app.load_destinations", side_effect=mock_load
...
class TestDestinationsService:
    ...
    def test_verify_token_success(self, client):
        ...
        response = client.get("/api/destinations/all", headers={"accessToken": token})
    ...
    def test_verify_token_invalid_format(self, client):
        ...
        response = client.get(
            "/api/destinations/all", headers={"accessToken": "invalid.token.format"}
        )
        assert response.status_code == 403
        assert "Invalid token" in response.get_json()["Message"]
    ...
    def test_create_destination_non_admin(self, client):
        ...
        token = generate_token(1, "User")
        data = {"name": "London", "description": "Capital of UK", "location": "Europe"}
    ...
    def test_get_destination_by_id(self, client, mock_destinations_db):
        ...
        token = generate_token(1, "User")
        response = client.get(
            "/api/destinations/1", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_get_destination_not_found(self, client, mock_destinations_db):
        ...
        token = generate_token(1, "User")
        response = client.get(
            "/api/destinations/999", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_delete_destination_success(
        self, client, mock_destinations_db, mock_file_operations
    ):
    ...
    def test_authorization_header_variations(self, client, mock_file_operations):
        ...
        token = generate_token(1, "Admin")
        data = {"name": "Test", "description": "Test Desc", "location": "Test Loc"}
